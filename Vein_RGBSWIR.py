...
class Runner(BaseRunner):
    ...
    def vein_inference(self, model, graph1, tube , batch_size):
        ...
        for j in range(0, image_height, patch_size):
            ...
            vein_mask_RGB[py1:py2, px1:px2] = cv2.resize(pred_squre, (patch_size, patch_size))
            patch_index += 1

        vein_mask_RGB = (vein_mask_RGB * 255).astype(np.uint8)
        ...
        #Apply SWIR Thresholdong on the binary mask generated by unet mode using  RGB images
        tubeID = tube.split('_')[-1]  # tube is `DH_tubeID`
        Vein_mask_filteredby_SWIR = Vein_SWIR_Threshold(self.SWIR_dir, vein_mask_RGB, tubeID, min_area=1000)
